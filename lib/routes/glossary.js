...
module.exports = function (app, dbs) {
    ...
    app.get('/glossary/:title', function (req, res) {
    ...
    app.post('/glossary/:title', [check("comment").isLength({ min: 10 }).isLength({ max: 256 }).trim(), check("author").isLength({ min: 2 }).isLength({ max: 20 }).trim()], function (req, res) {
        ...
        if (!errors.isEmpty()) {
            ...
            dbs.hoglarBlog.collection('glossary').updateOne(myquery, newvalues).then(function () {
                dbs.hoglarBlog.collection('glossary').find({ "topic": topic }).toArray(function (err, docs) {
